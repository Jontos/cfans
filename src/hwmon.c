...
int hwmon_device_init(sd_device_enumerator **enumerator, sd_device **device, const char *driver) {
  *enumerator = NULL;
  int ret;
...
int hwmon_source_init(Source *config, hwmonSource *source) {
  if (hwmon_device_init(&source->enumerator, &source->device, config->driver) < 0) {
    return -1;
  }
...
int hwmon_fan_init(Fan *config, hwmonFan *fan) {
  if (hwmon_device_init(&fan->enumerator, &fan->device, config->driver) < 0) {
    return -1;
  }
...
int hwmon_read_temp(hwmonSource *source) {
  ...
  for (int i = 0; i < source->num_inputs; i++) {
    ...
    if (sd_device_get_sysattr_value(source->device, source->temp_inputs[i], &value) >= 0) {
      int temp = (int)strtol(value, NULL, 0) / source->scale;
